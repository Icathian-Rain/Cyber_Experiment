...
class DHClient(DHProto):
    ...
    def handle_handshake_reply(self, pkt_data):
        print(pkt_data[4+DEFAULT_KEY_BYTES_LENGTH*2:].decode('utf-8'))
    ...
    def send_confirm_shared(self, addr):
        ...
        data = int.to_bytes(self.ya,DEFAULT_KEY_BYTES_LENGTH, byteorder = 'big')
        print(self.ya)
        print(header+data)
        self.sock.sendto(header + data, addr)
    ...
    def run(self):
        self.send_handshake_request(('10.0.2.7', 8000))
        while True:
            try:
                pkt_data, addr = self.sock.recvfrom(DEFAULT_BUFFER_SIZE)
                print(pkt_data)
